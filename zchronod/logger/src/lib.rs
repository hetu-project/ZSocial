...
struct zChronodLogger {}

impl log::Log for zChronodLogger {
...
///
/// # Examples
/// init_chrono_logger_with_path("./xx/chronod.log", "debug")
pub fn init_zchronod_logger_with_path(logfile_path: &str, filter: &str) {
...
fn replace_logger(logger: env_logger::Logger) {
    ...
    *CLOGGER.write().unwrap() = logger;
    let _ = log::set_boxed_logger(Box::new(zChronodLogger {}));
}

pub fn init_zchronod_logger() {
    init_zchronod_with_filter("error");
}

pub fn init_zchronod_with_filter(filter: &str) {
    let logger = env_logger::Builder::from_env(env_logger::Env::new().default_filter_or(filter))
        .format_timestamp_nanos()
        .build();
    replace_logger(logger);
